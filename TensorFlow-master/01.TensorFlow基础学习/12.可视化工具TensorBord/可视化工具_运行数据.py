...
with tf.Session() as sess:
    sess.run(init)
    writer = tf.summary.FileWriter(r'C:\Users\vcyber\Desktop\logs',sess.graph)
    for epoch in range(10):
        for batch in range(n_batch):
            batch_xs,batch_ys =  mnist.train.next_batch(batch_size)
            summar,_=sess.run([merge,train_step],feed_dict={x:batch_xs,y:batch_ys})
        
        writer.add_summary(summar, epoch)
