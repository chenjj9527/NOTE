...
class Jieba {
 public:
  ...
  void CutForSearchEx(const string& sentence, vector<string>& words, map<std::string,vector<std::string> >&subSentence,
  	                 bool hmm = true,bool delMoreSpace=false) const {
	string preStr;
	vector<string> tempWords;
	query_seg_.Cut(sentence, words,subSentence, hmm);

	if(delMoreSpace){
		/*去掉连续的空格分词，只保留一个空格分词*/
		for (size_t i = 0; i < words.size(); i++) {
			if((preStr==" ") && (preStr==words[i])){
				continue;
			}
			preStr = words[i];
			tempWords.push_back(words[i]);
		}
		words.swap(tempWords);
	}
  }
