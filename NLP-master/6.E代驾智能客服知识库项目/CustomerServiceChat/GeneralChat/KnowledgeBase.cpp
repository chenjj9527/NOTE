...
bool KnowledgeBase::initFromDB(const std::string& sCommonDicDir, acl::MySQLQuery& mySqlQuery,robosay::KeyWordSearch* pKeyWordSearch,const std::string& idField, const std::string& questionField){
	...
	//detail正文,questionValue标题,idValue是知识库的标号

	// 第一步连接数据库  
	while (!mySqlQuery.IsTraversalFinished())
	{
		int idValue = mySqlQuery.GetIntField(idField);
		std::string questionValue = mySqlQuery.GetStringField(questionField);
		std::string result;
		int isDelete = mySqlQuery.GetIntField("is_delete");
		int is_enable = mySqlQuery.GetIntField("is_enable");

		std::string detail = mySqlQuery.GetStringField("details");	

		if(!detail.empty()){
			//对正文格式进行处理,得到文本内容result
			m_Config.m_Parser.fliterHtmlTags(detail,result);
		}
		if(idValue==-1 || questionValue.empty() || (isDelete==1) || (is_enable==1)){
			mySqlQuery.ToNext();
			continue;
		}
		
		std::string answerValue = acl::Utility::int2Str(idValue);
		robosay::general::QA qa(m_vQA.size());
		if(qa.init(questionValue,answerValue) == false) {
			ELOGGER->print(true, "KnowledgeBase::initFromDB [qa.init(xml,qaPtr) == false] Failed to init QA node.");
			return false;
		}

		m_vQA.push_back(qa);
		//建立索引,一个ID号对应一个QA
		m_DBIdToQAId.insert(std::pair<int,int>(idValue,m_vQA.size()));
		if(!result.empty()){
			//对正文进行分词
			std::vector<std::string> vGetWord = m_Config.m_Parser.parseWithJiebaForShortWords(result).getWords();
			m_pKeyWordSearch->filterStopWord(vGetWord);
			//倒排索引
			m_pKeyWordSearch->m_InvertIndex.handleDocument(vGetWord,m_vQA.size()-1);
		}		
		mySqlQuery.ToNext();
	}
	...
	*/
	...
	ELOGGER->print(true, "record number on table is [%d].\n", m_vQA.size());
...
bool KnowledgeBase::initDefineTags(const std::string& sCommonDicDir,acl::MySQLQuery& mySqlQuery,const std::string& idField, const std::string& TagField){
	...
	while (!mySqlQuery.IsTraversalFinished())
	{
		int idValue  = mySqlQuery.GetIntField(idField);
		int isDelete = mySqlQuery.GetIntField("is_delete");
		std::string questionValue = mySqlQuery.GetStringField(TagField);

		if(idValue==-1 || questionValue.empty() || (isDelete==1)){
			mySqlQuery.ToNext();
			continue;
		}
		//找到对应的QA
		std::map<int,int>::const_iterator itr = m_DBIdToQAId.find(idValue);
		if(itr==m_DBIdToQAId.end() || itr->second>m_vQA.size()){
			mySqlQuery.ToNext();
			continue;
		}
		//插入标签
		m_vQA[itr->second-1].insertDefineTag(questionValue);
		tagNumber++;
	...
	*/
	...
	ELOGGER->print(true, "DefineTags record on table is [%d].\n", tagNumber);
...
bool KnowledgeBase::initIDF(){
	...
	m_pKeyWordSearch->m_InvertIndex.initTfIdf();

	std::map<std::string,std::vector<std::string> >subSentence;
	/*计算question的IDF*/
	for(size_t i=0; i<m_vQA.size(); i++){
		const std::vector<Sentence>& vQuestion = m_vQA[i].getQuestions();
		std::set<int>                wordToIntSet;
			...
			for(;itr!=allSegmentMethod.end();itr++){
				...
				for(size_t k=0; k<vWord.size(); k++){
					...
					if(iter == m_pKeyWordSearch->m_TFIDF.m_word_idx.end()){
						...
						wordToIntSet.insert(m_pKeyWordSearch->m_TFIDF.m_word_idx.size());
						...
						wordToIntSet.insert(iter->second);
		...
		m_vQA[i].setTitleSearchWords(wordToIntSet);
	}
...
bool KnowledgeBase::initPinyin(std::vector<QA>& vQA,robosay::XmlParser& xml, const std::string& sCommonDicDir){
	// 读入 HANZI-PINYIN的字典
	acl::File Hanzi2PinyinDic;
	std::string line, Hanzi, Pinyin;
	std::string sPinyinDic = "Hanzi2Pinyin.dic";
	if(sPinyinDic.empty()){
	...
	for(size_t i=0; i<vQA.size(); i++){
		...
		for(size_t j=0; j<vQuestion.size(); j++){
			...
			// 将输入字符串中包含的中文字符转换成拼音表示
			m_pKeyWordSearch->filterStopWord(vIn);
...
// 基于 Pinyin 的匹配
bool KnowledgeBase::searchByPinyin(const std::string& sIn, robosay::base::VResult& vResult, const std::string& sWay, std::set<int>& setMatched,
	...
	StringVector vIn = s.toCharactorWithLatin();
	m_pKeyWordSearch->filterStopWord(vIn);
	StringVector vWordPinyin = m_PINYIN.Hanzi2Pinyin(vIn);

	for(size_t i=0; i<m_vQA.size(); i++){
		std::set<int>::const_iterator itr = setMatched.find(i);
		if(itr!=setMatched.end()){
			continue;			
		}			
	...
	for(int i=0;iter!=elems.end();iter++,i++){
		...
		if(itr!=mapIndexQuestion.end()){
			...
			if(pMatchResult!=NULL && pMatchResult->PathInVResult(pRst)){
				std::cout<<"searchByPinyin result is in the MatchResult.skip it."<<stc.getSentence()<<std::endl;
				continue;
			}
...
bool KnowledgeBase::searchByTags(const std::string& sIn, robosay::base::VResult& vResult, const std::string& sWay, std::set<int>& setMatched){
	std::set<std::string> setTags = robosay::general::QA::getTagsInSentence(sIn);

	int questionIdx = 0;
	
	robosay::base::CSortManager<int,float>  sortManager;
	std::map<int,std::set<std::string> >  mapIndexQuestion;
	...
	for(size_t i=0; i<m_vQA.size(); i++){
		...
		std::set<std::string> setMatchTag;
		float fScore = m_vQA[i].matchByTags(setTags,setMatchTag);
		//std::cout << "======> fScore:" << fScore << std::endl;
		if(fScore > 0.01){ //0.680   0.35
			sortManager.Add(fScore,i);
			mapIndexQuestion.insert(std::make_pair(i,setMatchTag));
			setMatched.insert(i);
		}
	}
	...
	for(int i=0;iter!=elems.end();iter++,i++){
		std::map<int,std::set<std::string> >::const_iterator itr = mapIndexQuestion.find(iter->GetElem());
		if(itr!=mapIndexQuestion.end()){
			robosay::base::Result* pRst = createResult();
			scoped_ptr<robosay::base::Result> autoPtrObj(pRst);	
			if(pRst==NULL)
			...
			{
				std::string result;
				std::set<std::string>::const_iterator it = itr->second.begin();
			...
			pRst->setQuestionType(Question_info::tagsAnwser);		
	        autoPtrObj.detach();
	        vResult.push_back(pRst);			
		}
	}
...
bool KnowledgeBase::searchByInvertIndex(const std::string& sIn, robosay::base::VResult& vResult, const std::string& sWay,
									 std::set<int>& setMatched,std::map<int,std::string>& highWordMatchQA){
	int questionIdx = -1;
	...
	for(int i=0;i<invertResult.size()&& i<50;i++){
		...
		if(pRst==NULL)
			return false;
		pRst->m_pPath->setPath(&m_vQA[invertResult[i].first]);
		
		//pRst->setScore(iter->GetWeight());
		StringStringMap mEmpty;
		//pRst->setRelateQuestions(getRelateQuestion(iter->GetElem(), sIn));
		pRst->setAnswerInfo(m_vQA[invertResult[i].first].getAnswerInfo(mEmpty, NULL, sWay));
		//const Sentence& stc = m_vQA[invertResult[i].first].getQuestion(itr->second);
		
		//m_vQA[qaIndex].getDetailSameWords(setWordId, vResult[uIdx]->m_sMatchDetailWords)

		//pRst->m_sMatchDetailWords =  invertResult[i].second;
		pRst->setTarget(invertResult[i].second, "");

		pRst->setQuestionType(Question_info::detailAnwser);		
		autoPtrObj.detach();
		vResult.push_back(pRst);			
	}
...
bool KnowledgeBase::execute(const std::string& sText, robosay::base::VResult& vResult, const std::string& sWay, 
							const std::vector<std::vector<TfidfValue> >& vInTfidf, const std::map<int,std::string>& mapIdToWord){

	robosay::base::Input* input = createInput();
	scoped_ptr<robosay::base::Input> autoPtrObj(input);
	input->m_sIn = sText;
	chat(*input,vResult, sWay,vInTfidf,mapIdToWord);
	if(vResult.empty()){
		vResult.sMessage = "[ERROR]:未找到对应的answer";
		return false;
	}
...
bool KnowledgeBase::chat(robosay::base::Input& input,robosay::base::VResult& vResult, const std::string& sWay,
	...
	std::set<int> setWordId;
	std::set<int> setMatched;
	

	std::map<int,std::string>::const_iterator itr = mapIdToWord.begin();
	for(;itr!=mapIdToWord.end();itr++){
		setWordId.insert(itr->first);
	}

	searchByTags(input.m_sIn,vMidRslt,sWay,setMatched);
	if(!vMidRslt.empty()){
	...
	searchByTFIDF(input.m_sIn, vMidRslt, sWay,vInTfidf,setMatched);
	if(!vMidRslt.empty()){
		vResult.moveFrom(vMidRslt);
		//ELOGGER->print(true, "KnowledgeBase::chat [!vMidRslt.empty()]");
		//return true;
	...
	// 汉字转拼音，然后再进行基于拼音的匹配
	searchByPinyin(input.m_sIn, vMidRslt, sWay,setMatched,&vResult);
	if(!vMidRslt.empty()){
		vResult.moveFrom(vMidRslt);
	}	
	...
	//vResult.sort();
	vResult.reduceSize(robosay::base::Result::max_result_number);	
	...
	for(size_t uIdx = 0u; uIdx < vResult.size(); ++uIdx){
		...
		int qaIndex = vResult[uIdx]->m_pPath->GetQAInx();
		if(qaIndex==-1) continue;

		if(!vResult[uIdx]->getQuestionInfo().get_type().empty()){
			m_vQA[qaIndex].getTitleSameWords(setWordId, vResult[uIdx]->m_sMatchTitleWords);

			std::map<int,std::string>::const_iterator itr = highWordMatchQA.find(qaIndex);
			if(itr!=highWordMatchQA.end()){
				vResult[uIdx]->m_sMatchDetailWords = itr->second;
			}
			//m_vQA[qaIndex].getDetailSameWords(setWordId, vResult[uIdx]->m_sMatchDetailWords);				
		}
	}
...
bool KnowledgeBase::chat(robosay::base::Input& input,robosay::base::VResult& vResult, const std::string& sWay){
	...
	// 汉字转拼音，然后再进行基于拼音的匹配
	searchByPinyin(input.m_sIn, vMidRslt, sWay,setMatched);
	if(!vMidRslt.empty()){
		vResult.moveFrom(vMidRslt);
		//ELOGGER->print(true, "KnowledgeBase::chat [!vMidRslt.empty()]");
		//return true;
