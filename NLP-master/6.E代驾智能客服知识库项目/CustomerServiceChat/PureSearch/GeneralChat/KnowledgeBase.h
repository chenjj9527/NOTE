...
#include "CustomerServiceChat/GeneralChat/PINYIN.h"
...
public:
	...
	bool initIDF(robosay::KeyWordSearch* pKeyWordSearch);
	bool calculateTfidf();
    ...
    bool execute(const std::string& sText, bool bNeedAnalysis, robosay::base::VResult& vResult, const std::string& sWay, 
	...
	bool initFromDB(acl::String FileName);
...
private:
	...
	bool initPinyin(std::vector<QA>& vQA,robosay::XmlParser& xml, std::string sCommonDicDir);         // 初始化PINYIN操作，加载数据
	...
	bool searchByPinyin(const std::string& sIn, robosay::base::VResult& vResult, const std::string& sWay);                  // Pinyin 匹配
